...
export function applyEdits(record,storage){
  try{const e=JSON.parse(storage.getItem('yao.collection.edits.v1')||'{}')[record.patternId];if(!e)return record;
    ...
    for(const key of ['patternName','patternSource','branch','image','backImage','photoAssetId','backPhotoAssetId','story','technique'])if(typeof p[key]==='string')r[key]=p[key];
    ...
    if(typeof p.image==='string'&&p.image!==record.image){r.arTargetId=undefined;if(r.relief)r.relief={...r.relief,arTargetId:undefined};
      if(typeof p.photoAssetId!=='string')r.photoAssetId='';
      if(typeof p.backImage!=='string'){r.backImage='';r.backPhotoAssetId='';}
    }
    if(typeof p.backImage==='string'&&p.backImage!==record.backImage&&typeof p.backPhotoAssetId!=='string')r.backPhotoAssetId='';
    return r;
  }catch{return record;}
}
